...
public class BookBuyAdapter extends BaseAdapter {
    ...
    public BookBuyAdapter(Context mContext, ArrayList<BookSell> arrayList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        imgAdd.setOnClickListener(view ->{
            databaseReference.child(phone).child("cart").child(arrayList.get(position).getBookSetName()).setValue(arrayList.get(position));
            imgAdd.setVisibility(View.GONE);
            imgRemove.setVisibility(View.VISIBLE);
        });

        imgRemove.setOnClickListener(view ->{
            databaseReference.child(phone).child("cart").child(arrayList.get(position).getBookSetName()).removeValue();
            imgAdd.setVisibility(View.VISIBLE);
            imgRemove.setVisibility(View.GONE);
        });

        uniSetName.setText(arrayList.get(position).getBookSetName());
        uniClass.setText(arrayList.get(position).getClassName());
        uniSchool.setText(arrayList.get(position).getSchoolName());
        totalBook.setText(String.valueOf(arrayList.get(position).getBookNo()));
        totalPrize.setText(String.valueOf(arrayList.get(position).getTotal()));

        return convertView;
    }

    public void filterList(ArrayList<BookSell> s)
    {
        arrayList = s ;
